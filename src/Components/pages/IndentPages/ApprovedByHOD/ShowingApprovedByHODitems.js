...
const HeaderCardItems = ["Item Name","UOM","Par Level","Current Inventory","Requested Qt.", "Remark"];
...
function ItemCard({src,parlevel,UOM,currentInventory,name,index,quantity,requestedQuantity,}) {
    ...
    const [count, setCount] = useState(countinitial ? countinitial : 1);
    ...
    <div className="font-sans">
      <div className={`grid grid-cols-6 sm:grid-cols-6 md:grid-cols-6 mt-1 w-full lg:h-[48px] h-[40px] 
        ...
        <div className=" pl-2 flex justify-center items-center text-blue-500">
          <span className="text-left w-10">{UOM}</span>
        </div>
        <div className=" pl-2 flex justify-center items-center text-blue-500">
...
function ShowApprovedIndentItems() {

    const [draftitemstoshow, setDraftitemstoshow] = useState([]);
    const {existingdraft, rawIngredients, inventoryItems, image, uom, parlevel, updateToIssued, updateStatustoDraft} = useAuth();
    const [kitchen,setKitchen] = useState("");
    const [date,setDate] = useState("");
    const [raisedby,setRaisedby] = useState("");
    const [dbid, setDbid] = useState();
    const [role, setRole] = useState("Store"); //Kitchen //Store
  ...
  function handleConvetToDraft() {
    updateStatustoDraft(dbid);
    navigate("/Indent/Drafting")
  }

  function handleBack () {
    navigate("/Indent/ApproveByHOD");
  }
  ...
  return (
    <div>
        <div class="flex">
          <div class="text-xl font-semibold mb-[5px] cursor-pointer" onClick={handleBack} >Approved </div>
        ...
        {(role === "Store") && <div class="h-auto flex justify-end mt-4">
            
            <div class="flex gap-4">
              <div ><WhiteButton text={"Hold"}/></div>
              <div onClick={() => (handleOpenModal(),handleUpdate())}>
                 <BlueButton text={"Issue Items"}/>
              </div>
              <Modal isOpen={isModalOpen} onClose={handleCloseModal} onOkay={handleOkayModal} noOfItems={draftitemstoshow.length} kitchen={"kitchen"} setShowitems={() => (console.log("Hogaya"))} Message={"Issued Successfully"}/>
            </div>
        </div>}
...
export default ShowApprovedIndentItems;